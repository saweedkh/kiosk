"""
Re-seed POS/printer connection fields from env when SiteSettings still has
empty/default values. Covers installs where 0020 ran without .env mounted.
"""
...
def reseed_connection_from_env(apps, schema_editor):
    ...
    if gw in ('pos', 'direct') and (site.payment_mode or 'mock') == 'mock':
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('core', '0021_pos_protocol_settings'),
    ]

    operations = [
        migrations.RunPython(reseed_connection_from_env, noop_reverse),
    ]
