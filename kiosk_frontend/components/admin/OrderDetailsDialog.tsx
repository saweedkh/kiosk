...
export function OrderDetailsDialog({
  ...
  useEffect(() => {
    ...
    if (order?.payment_status) setNextPaymentStatus(order.payment_status)
  ...
  const invalidateOrderQueries = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ['exception-report'] })
  }
  ...
  const updatePaymentMutation = useMutation({
    mutationFn: async (paymentStatus: string) => {
      if (!orderId) throw new Error('order id missing')
      return ordersApi.updateAdminOrderPaymentStatus(orderId, paymentStatus)
    },
    onSuccess: invalidateOrderQueries,
  })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90vh] flex-col overflow-hidden p-0 sm:max-w-2xl">
        <DragScrollArea className="max-h-[90vh] space-y-4 p-6">
          <DialogHeader>
            <DialogTitle className="text-right text-xl font-black">جزئیات سفارش</DialogTitle>
          ...
          {isLoading || (isFetching && !order) ? (
            ...
            <div className="space-y-5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  سفارش: {statusLabel(order.status)}
                </span>
                <span
                  className={cn(
                    'rounded-full px-3 py-1 text-xs font-bold',
              ...
              <div className="rounded-2xl border border-border/80 bg-muted/25 px-4">
                <MetaRow
                  label="شماره سفارش"
                  value={<span className="font-mono text-sm">{order.order_number}</span>}
                />
                {order.receipt_number != null && Number(order.receipt_number) > 0 ? (
                  <MetaRow label="شماره فیش" value={formatNumber(order.receipt_number)} />
                ) : null}
                <MetaRow label="تاریخ" value={formatJalaliDateTime(order.created_at)} />
              ...
              {canEditStatus ? (
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="space-y-3 rounded-2xl border border-border/80 bg-muted/20 p-4">
                    <p className="text-sm font-bold text-foreground">وضعیت سفارش</p>
                    <select
                      value={nextStatus}
                    ...
                    <Button
                      ...
                      className="w-full"
                      disabled={
                        updateStatusMutation.isPending || !order || nextStatus === order.status
                      }
                      onClick={() => updateStatusMutation.mutate(nextStatus)}
                    >
                      {updateStatusMutation.isPending ? 'در حال ذخیره...' : 'ذخیره وضعیت سفارش'}
                    </Button>
                    {updateStatusMutation.isError ? (
                      <p className="text-xs text-red-600 dark:text-red-400">
                        {translateError(updateStatusMutation.error) ||
                  ...
                  <div className="space-y-3 rounded-2xl border border-border/80 bg-muted/20 p-4">
                    <p className="text-sm font-bold text-foreground">وضعیت پرداخت</p>
                    <select
                    ...
                    <Button
                      ...
                      onClick={() => updatePaymentMutation.mutate(nextPaymentStatus)}
                    >
                      {updatePaymentMutation.isPending
                        ? 'در حال ذخیره...'
                        : 'ذخیره وضعیت پرداخت'}
                    ...
                    {updatePaymentMutation.isError ? (
                      <p className="text-xs text-red-600 dark:text-red-400">
                        {translateError(updatePaymentMutation.error) ||
